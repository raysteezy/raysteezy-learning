#!/usr/bin/env python3
"""
Planet Labs (PL) — Monte Carlo Simulation v2
=============================================
Upgraded from the v1 constant-parameter GBM to include:

  v1 (kept as baseline):
    - Constant drift and volatility GBM
    - Static multiplier stress tests
  ...
  v2 (new):
    ...
    - Improved walk-forward validation and bootstrap CIs
    - Combined model that blends GBM + stochastic vol + jumps
...
# =====================================================================
# STEP 2: V1 Baseline — Standard GBM (kept for comparison)
# =====================================================================
...
print(f"  v1 2yr median: ${np.median(v1_terminal):.2f}")


# =====================================================================
# STEP 3: V2 — Stochastic Volatility (Heston-inspired)
# =====================================================================
...
# - theta: the long-term average variance (sigma_daily² roughly)
...
#         when price drops, volatility tends to spike)
...
rho = float(log_returns.corr(                      # leverage effect
...
for t in range(FORECAST_DAYS):
    ...
    # Variance can't go negative — use max(variance, 0) (truncation scheme)
    ...
    variance = var_pos + kappa * (theta - var_pos) * dt + xi * vol * np.sqrt(dt) * z2
...
# =====================================================================
# STEP 4: V2 — Jump Diffusion (Merton model)
# =====================================================================

print(f"\n[4/7] Running v2 jump diffusion model ...")

# Jump parameters estimated from PL's tail behavior:
# - lam (lambda): expected number of jumps per year
# - jump_mu: average jump size (in log returns)
# - jump_sigma: spread of jump sizes
#
# I detect jumps as returns that are more than 3 standard deviations
# from the mean. This gives us real numbers from PL's actual history.
...
# Compensator: adjust drift so the jumps don't bias the expected return
jump_compensator = lam * (np.exp(jump_mu + 0.5 * jump_sigma ** 2) - 1) / 252
...
for t in range(FORECAST_DAYS):
    ...
    # Total jump size for today (sum of all jumps)
    ...
    # GBM part + jump part
    continuous = (mu_daily - 0.5 * sigma_daily ** 2 - jump_compensator) * dt
    diffuse = sigma_daily * np.sqrt(dt) * z
    paths_jd[t + 1] = paths_jd[t] * np.exp(continuous + diffuse + jump_sizes)
...
# a Hidden Markov Model to automatically find the real market regimes
# in PL's history. The HMM learns which days were "calm" and which
# were "volatile" and gives us the actual drift/volatility for each regime.

try:
    from hmmlearn.hmm import GaussianHMM

    # Fit a 2-state HMM on daily returns
    ...
    regime_probs = hmm.startprob_

    # Label regimes (the one with lower vol is "calm", higher vol is "volatile")
    ...
    print(f"  Calm regime:     mu={hmm_regimes['calm']['mu_annual']:.2%}, "
          f"vol={hmm_regimes['calm']['sigma_annual']:.2%}, "
          f"({hmm_regimes['calm']['frequency']:.0f}% of days)")
    print(f"  Volatile regime: mu={hmm_regimes['volatile']['mu_annual']:.2%}, "
          f"vol={hmm_regimes['volatile']['sigma_annual']:.2%}, "
          f"({hmm_regimes['volatile']['frequency']:.0f}% of days)")

    hmm_available = True
except Exception as e:
    print(f"  HMM fitting failed ({e}), falling back to v1 multiplier-based scenarios")
    hmm_available = False
    hmm_regimes = None

# Build stress-test scenarios using real regime parameters
# If HMM works, base the scenarios on actual detected regimes
# If HMM fails, fall back to v1-style multipliers
...
if hmm_available:
    ...
    SCENARIOS = {
        "market_crash": {
            "name": "Market Crash (2008-style)",
            "description": "Severe downturn — negative drift, high vol",
...
# Walk-forward validation (same as v1 but now also testing Heston)
...
for lb in lookbacks:
    ...
    for start in range(lb, len(close) - 21, 63):
        ...
        # GBM prediction
        pred_gbm = float(start_price * np.exp((w_mu - 0.5 * w_sigma ** 2) * 21))
        errors_gbm.append(np.log(actual / pred_gbm))

        # Heston prediction (simplified — use mean-reverted vol)
        w_var = w_sigma ** 2
        heston_var_21 = w_var + kappa * (theta - w_var) * (21 / 252)
        heston_vol = np.sqrt(max(heston_var_21, 0))
        pred_heston = float(start_price * np.exp((w_mu - 0.5 * heston_var_21) * 21))
    ...
    print(f"  {lb}-day lookback: GBM RMSE={wf_results[f'{lb}_day_lookback']['gbm']['rmse']:.4f}, "
          f"Heston RMSE={wf_results[f'{lb}_day_lookback']['heston']['rmse']:.4f}  (n={n_tests})")
...
mu_ci = [round(float(np.percentile(boot_mus, 2.5)), 6),
         round(float(np.percentile(boot_mus, 97.5)), 6)]
sigma_ci = [round(float(np.percentile(boot_sigmas, 2.5)), 6),
...
ax.set_title(f"Planet Labs ({TICKER}) — Monte Carlo v1 vs v2 ({N_SIMULATIONS:,} paths)",
             fontsize=16, fontweight="bold", pad=15)
ax.set_xlabel("Date", fontsize=12)
ax.set_ylabel("Stock Price (USD)", fontsize=12)
ax.legend(loc="upper left", fontsize=8, framealpha=0.7)
fig.text(0.5, 0.01,
         "v2 adds stochastic volatility (Heston) + jump diffusion (Merton)  |  Not financial advice.",
...
hmm_label = "HMM regime-based" if hmm_available else "Static multipliers (v1)"
...
# Panel 1: Comparison of terminal distributions (v1 GBM vs v2 Heston vs v2 JD)
ax_dist = axes[0, 0]
bins = np.linspace(0, 150, 200)
ax_dist.hist(v1_terminal, bins=bins, alpha=0.3, color="#888888",
             edgecolor="none", label=f"v1 GBM (med ${np.median(v1_terminal):.0f})")
ax_dist.hist(heston_terminal, bins=bins, alpha=0.4, color="#00d4aa",
             edgecolor="none", label=f"v2 Heston (med ${np.median(heston_terminal):.0f})")
ax_dist.hist(jd_terminal, bins=bins, alpha=0.3, color="#b388ff",
             edgecolor="none", label=f"v2 Jump Diff (med ${np.median(jd_terminal):.0f})")
ax_dist.axvline(current_price, color="#FFA500", linewidth=2,
                label=f"Current ${current_price:.2f}")
ax_dist.set_title("2yr Terminal Distributions: v1 vs v2", fontsize=13, fontweight="bold")
...
ax_heat.set_title("Parameter Sensitivity (2yr Median)", fontsize=13, fontweight="bold")
...
ax_tbl = axes[1, 1]
ax_tbl.axis("off")
ax_tbl.set_title("Model Comparison (2-Year)", fontsize=13, fontweight="bold", pad=20)
...
table_data = [
    ...
    ["VaR 95%",
     f"${v1_rm['var_95']}", f"${h_rm['var_95']}", f"${jd_rm['var_95']}"],
    ["CVaR 95%",
     f"${v1_rm['cvar_95']}", f"${h_rm['cvar_95']}", f"${jd_rm['cvar_95']}"],
    ["P(Profit)",
     f"{v1_rm['prob_profit']}%", f"{h_rm['prob_profit']}%", f"{jd_rm['prob_profit']}%"],
    ["P(Double)",
     f"{v1_rm['prob_double']}%", f"{h_rm['prob_double']}%", f"{jd_rm['prob_double']}%"],
]
...
fig3.suptitle(f"Planet Labs ({TICKER}) — Robustness & Risk Analysis (v2)",
              fontsize=16, fontweight="bold", y=0.98)
fig3.text(0.5, 0.01,
          f"{N_SIMULATIONS:,} paths per model  |  {BOOTSTRAP_SAMPLES:,} bootstrap samples  |  Not financial advice.",
          ha="center", fontsize=8, alpha=0.5)
...
summary = {
    ...
    "regime_detection": hmm_regimes if hmm_available else "HMM not available — used v1 fallback",
    ...
    "disclaimer": (
        ...
        "Models assume specific parametric forms that may not hold in practice."
    ),
}
...
# Percentile paths CSV (using Heston as the primary model now)
...
print(f"v2 Heston 2yr median:        ${np.median(heston_terminal):.2f}")
print(f"v2 Jump Diffusion 2yr median: ${np.median(jd_terminal):.2f}")
print(f"P(Profit) — GBM: {v1_rm['prob_profit']}%  Heston: {h_rm['prob_profit']}%  JD: {jd_rm['prob_profit']}%")
